...
var guildid, channelid, waynotify, webhookurl, usernotify, autocaptcha, apiuser, apikey, musicpath, gemorder;
...
function getAccount() {
    return [{
        ...
        when: answer => {
            return answer.username !== undefined;
        }
    }, {
        type: "input",
        name: "mfaCode",
        ...
        when: answer => {
            return answer.password !== undefined;
        }
    }];
}
...
function resolveData(tag, token, guildID, channelID = [], wayNotify = [], musicPath, webhookURL, userNotify, captchaAPI, apiUser, apiKey, autoDaily, autoPray, autoSlash, autoGem, autoLootbox, gemOrder, autoQuote, autoOther, autoRefresh, autoSleep, autoWait) {
    return {
        tag,
        token,
        guildID,
...
Please note that we do not take any responsibility for accounts being banned due to the use of our tools.`
    ...
    while(!client) {
        ...
        if (account === 0) {
            ...
            const profile = await getResult(getAccount());
            log("Checking Account...", "i");
            client = await accountCheck([profile.username, profile.password, profile.mfaCode]);
        } else if(account === 3) {
    ...
    conf = resolveData(client.user.tag, client.token, guildid, channelid, waynotify, musicpath, webhookurl, usernotify, autocaptcha, apiuser, apikey, autodaily, autopray, autoslash, autogem, autolootbox, gemorder, autoquote, autoother, autorefresh, autosleep, autowait)
