...
import { timer } from "../src/SelfbotWorker.js";
import { config, FolderPath } from "../index.js";
...
export function accountRemove(id, data) {
    delete data[id];
    fs.writeFileSync(FolderPath, JSON.stringify(data), "utf8")
}
...
export async function solveCaptcha(url) {
    ...
    const postData = {
        userid: config.apiUser,
        apikey: config.apiKey,
        data: base64_encode(Path),
    }
    return new Promise(async (resolve, reject) => {
        const response = await axios.post('https://api.apitruecaptcha.org/one/gettext', postData).catch(reject);
